  ...
  <div class="containers" :class="{ resizing }" :style="style">
    ...
    <el-collapse v-model="activeCollapseNames">
      <el-collapse-item title="RectInfo" name="1" v-if="currentViewInfo.rect">
        <div
          class="info-item"
          v-for="(value, key) in currentViewInfo.rect"
          :key="key"
        >
      ...
      <el-collapse-item title="Style" name="2" v-if="currentViewStyles&&currentViewInfo.viewId!==undefined">
        <el-row
          class="style-item"
          v-for="(item, index) in currentViewStyles"
          :key="index"
        >
          <el-col :span="2">
            <el-checkbox
              v-model="item.checked"
              @change="styleChecked($event, index)"
            ></el-checkbox>
          </el-col>
          <el-col :span="8">
            ...
            <el-input
              class="item-input"
              :class="{ 'item-input-active': item.keyEditable }"
              v-model="item.key"
              @click.native="handleStyleItem('editKey', index, item)"
              @focus="$event.target.select()"
              @blur="keyEditblur('keyEditable', index, item)"
            ></el-input>
          </el-col>
          <el-col :span="6" :offset="1">
            ...
            <el-input
              class="item-input"
              :class="{ 'item-input-active': item.valEditable }"
              v-model="item.value"
              @click.native="handleStyleItem('editValue', index, item)"
              @focus="$event.target.select()"
              @blur="keyEditblur('valEditable', index, item)"
            ></el-input>
          </el-col>
          <el-col :span="8" :offset="1">
            ...
            <el-button
              v-if="currentViewStyles.length - 1 === index"
              type="primary"
              icon="el-icon-plus"
              size="mini"
              circle
              @click="handleStyleItem('plus', index, item)"
            ></el-button>
          </el-col>
        </el-row>
        <el-row>
          <el-button
            v-if="
              currentViewStyles.length === 0 && currentViewInfo.viewId !== 0
            "
            type="primary"
            icon="el-icon-plus"
            size="mini"
            circle
            @click="handleStyleItem('plus')"
          ></el-button>
        </el-row>
      </el-collapse-item>
...
export default {
    ...
    "currentViewInfo.style": {
      handler(styleObj) {
        if (styleObj && Object.keys(styleObj).length) {
          ...
          this.lastTimeCurrentViewStyles = JSON.parse(
            JSON.stringify(this.currentViewStyles)
          );
        } else {
          this.currentViewStyles = [];
          this.lastTimeCurrentViewStyles = [];
        }
      },
      deep: true,
    },
  ...
  methods: {
    ...
    setViewStyle(setDefault, index) {
      ...
      setDefault &&
        (style[this.lastTimeCurrentViewStyles[index]?.key] =
          styleDefault[this.lastTimeCurrentViewStyles[index]?.key]);
      if (
        JSON.stringify(this.lastTimeCurrentViewStyles) !==
        JSON.stringify(this.currentViewStyles)
      ) {
        this.lastTimeCurrentViewStyles = JSON.parse(
          JSON.stringify(this.currentViewStyles)
